			       ...
			       u32 bmp_load_addr, size_t read_size)
{
	...
	case SPLASH_STORAGE_SF:
		return splash_sf_read_raw(bmp_load_addr, offset, read_size);	//走这里OK
	default:
			    ...
			    struct splash_location *locations, uint size)
{
	...
	env_splashsource = env_get("splashsource");		//获取显示的存储介质名字,sf
	if (env_splashsource == NULL)
		return &locations[0];		// 默认是sf
...
int splash_source_load(struct splash_location *locations, uint size)
{
	...
	env_splashimage_value = env_get("splashimage");		//获取存储位置
	if (env_splashimage_value == NULL)
		return -ENOENT;

	bmp_load_addr = simple_strtoul(env_splashimage_value, 0, 16);	//转化为log型数据
	if (bmp_load_addr == 0) {
		printf("Error: bad splashimage address specified\n");
		return -EFAULT;
	}

	splash_location = select_splash_location(locations, size);		//获取存储介质类型，得到名字
	if (!splash_location)
		return -EINVAL;

	if (splash_location->flags == SPLASH_STORAGE_RAW)
		return splash_load_raw(splash_location, bmp_load_addr);			//走这里
