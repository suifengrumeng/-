...
static void *video_logo(void)
{
	...
	splash_get_pos(&video_logo_xpos, &video_logo_ypos);		//获取显示位置

#ifdef CONFIG_SPLASH_SCREEN
	s = env_get("splashimage");				//splashimage保存的是内存上的地址
	if (s != NULL) {
		ret = splash_screen_prepare();		//从存储介质上读取bmp图片信息
		if (ret < 0)
	...
	logo_plot(video_fb_address, video_logo_xpos, video_logo_ypos);		//画图
	...
	sprintf(info, " %s", version_string);			//uboot的信息写在屏幕上
	...
	if (len > space) {
		...
		video_drawstring(VIDEO_INFO_X, VIDEO_INFO_Y, (uchar *) info);		//写字到屏幕上
...
unsigned char *c_bmphead;
/*
dest = (unsigned short *)video_fb_address + y * VIDEO_LINE_LEN + x * VIDEO_PIXEL_SIZE;
*/


extern  struct spi_flash *flash;

int show_lcd_logo(void)
{
	
	unsigned short *src; 
	unsigned char *p_lcd;
//	unsigned short *p_lcd_t;
	int i, j;
//	unsigned char *p_lcdsrt;
	//unsigned char *p_lcdsrt_t;
//	unsigned short red;
//	unsigned short green;
//	unsigned short blue;

//	char nand_cmd[50];
//	unsigned long bmp_from = 0;
//	size_t bmp_size = 0;

//	int retlen = 0;
 	src =  (unsigned short *)gImage_booting;

	printf("video_fb_address = 0x%p\n", video_fb_address);

	lcd_base = (unsigned char *)video_fb_address;
 
//	n = 120; 
	
//	p_lcd = lcd_base + (793-n)*1024*2;
    ...
    for(i=540;i<585;i++)
    {
        ...
        bmp_size = 0x00200000;
		p_lcdsrt =0xc2000000;
		memset(p_lcdsrt, 0, 800*600*2);

		nand_read_options_t opts;
		memset(&opts, 0, sizeof(opts));
		opts.buffer = (u_char*) p_lcdsrt;
		opts.length = 0x00200000;
		opts.offset = 0x00200000;
		opts.quiet		= 0;
		...
		spi_flash_read(flash, 0x400000, 0x200000, (u_char*)p_lcd);

			//nand_read_opts();
		
		 //printf("*bmphead=0x%x\n",*p_lcdsrt);
		//nand_read(&nand_info[0], 0x00200000, 0x00200000,  p_lcdsrt);
		
		//printf("%s\n", nand_cmd);
		//run_command(nand_cmd, 0);
		
		#if 0
		run_command("nand read 0xc0000000 0x00100000 0x00300000", 0);
		for(i = 0; i < 1000000; i++)
		{
			;
		}
        ...
        //ret = copy_kernel_img(p_lcdsrt,(char*)bmp_from,bmp_size,3);
		...
		ret = nand_legacy_rw (nand_dev_desc + curr_device,
								  ...
								  (unsigned char *) p_lcdsrt);
		*/
		bmphead=(struct BMPHEAD *)malloc(sizeof(bmphead));
        printf("bmphead=0x%x\n",bmphead);
        c_bmphead = p_lcdsrt;
		 printf("*bmphead=0x%x\n",*bmphead);
	#if 0
        ...
        bmphead->bfType = (*c_bmphead)*256 + *(c_bmphead+1);
        c_bmphead += 2;
        bmphead->bfSize =(*c_bmphead)+(*(c_bmphead+1))*256+(*(c_bmphead+2))*256*256+(*(c_bmphead+3))*256*256*256;
		#if 0
        printf("0=%x\n",*c_bmphead);
        printf("1=%x\n",*(c_bmphead+1));
        printf("2=%x\n",*(c_bmphead+2));
        printf("3=%x\n",*(c_bmphead+3));
		#endif
        c_bmphead =c_bmphead+4;
        bmphead->bfReserved = (*c_bmphead)+(*(c_bmphead+1))*256+(*(c_bmphead+2))*256*256+(*(c_bmphead+3))*256*256*256;
        c_bmphead += 4;
        bmphead->bfOffBits = (*c_bmphead)+(*(c_bmphead+1))*256+(*(c_bmphead+2))*256*256+(*(c_bmphead+3))*256*256*256;
        c_bmphead += 4;
        bmphead->biSize = (*c_bmphead)+(*(c_bmphead+1))*256+(*(c_bmphead+2))*256*256+(*(c_bmphead+3))*256*256*256;
        c_bmphead += 4;
        bmphead->biWidth = (*c_bmphead)+(*(c_bmphead+1))*256+(*(c_bmphead+2))*256*256+(*(c_bmphead+3))*256*256*256;
        c_bmphead += 4;
        bmphead->biHeight = (*c_bmphead)+(*(c_bmphead+1))*256+(*(c_bmphead+2))*256*256+(*(c_bmphead+3))*256*256*256;
        c_bmphead += 4;
        bmphead->biPlanes = (*c_bmphead)+(*(c_bmphead+1))*256;
        c_bmphead += 2;
        bmphead->biBitCount = (*c_bmphead)+(*(c_bmphead+1))*256;
        c_bmphead += 2;
        bmphead->biCompression = (*c_bmphead)+(*(c_bmphead+1))*256+(*(c_bmphead+2))*256*256+(*(c_bmphead+3))*256*256*256;
        c_bmphead += 4;
        bmphead->biSizeImage = (*c_bmphead)+(*(c_bmphead+1))*256+(*(c_bmphead+2))*256*256+(*(c_bmphead+3))*256*256*256;
        c_bmphead += 4;
        bmphead->biXPelsPerMeter = (*c_bmphead)+(*(c_bmphead+1))*256+(*(c_bmphead+2))*256*256+(*(c_bmphead+3))*256*256*256;
        c_bmphead += 4;
        bmphead->biYPelsPerMeter = (*c_bmphead)+(*(c_bmphead+1))*256+(*(c_bmphead+2))*256*256+(*(c_bmphead+3))*256*256*256;
        c_bmphead += 4;
        bmphead->biClrUsed = (*c_bmphead)+(*(c_bmphead+1))*256+(*(c_bmphead+2))*256*256+(*(c_bmphead+3))*256*256*256;
        c_bmphead += 4;
        bmphead->biClrImportant = (*c_bmphead)+(*(c_bmphead+1))*256+(*(c_bmphead+2))*256*256+(*(c_bmphead+3))*256*256*256;
        c_bmphead += 4;
        ...
        if(bmphead->bfOffBits<10000)
        {
			
	
        p_lcdsrt= (unsigned char *)(0xc2000000+(bmphead->bfOffBits));
		
     //   p_lcd = (unsigned short *)0x33f00000;
        p_lcd = (unsigned short *)(0xc2300000);
      

       for(i = 0; i< 600; i++)
       for(j = 0; j< 800; j++)
       {
        p_lcdsrt = (unsigned char *)(0xc2000000+(bmphead->bfOffBits)+ 1440000 - (800-j)*3 - (i*800)*3 );
        blue  = *p_lcdsrt;
        green = *(p_lcdsrt+1);
        red  = *(p_lcdsrt+2);
        *p_lcd = ((red>>3)<<11)|((green>>2)<<5)|( blue>>3);
         p_lcd++;
       }
   		
        p_lcd = (unsigned short *)(0xc2300000);

		//p_lcd_t = (unsigned short *)0xc1000000;
		p_lcd_t = (unsigned short *)lcd_base;
	...
	//	printf("4p_lcd_t =%x \n",p_lcd_t);
		...
		//memcpy(lcd_base, p_lcd, 800*600*2);
		//memset(p_lcd, 0, 0x00200000);
		#if 0
		for(i = 0; i < 768; i++)
		{
			for(j = 0 ; j < 1024; j++){
		...
		p_lcd_t = (unsigned short *)lcd_base;
		//p_lcd_t = (unsigned short *)0xc1000000;
       // lcd_init((unsigned short *)p_lcd_t);
		//lcd_init(lcd_base);
      }
...
static int cfg_video_init(void)
{
	...
	video_console_address = video_logo();       //LOGO
#else
