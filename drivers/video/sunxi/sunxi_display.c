...
static void sunxi_frontend_init(void) {}
static void sunxi_frontend_mode_set(const struct ctfb_res_modes *mode,
				    unsigned int address) {	printf("NULL --------->\n");}
static void sunxi_frontend_enable(void) {}
#endif
...
//	printf("2. overscan_x = %d, overscan_y = %d, sunxi_display.monitor = %d\n", overscan_x, overscan_y, sunxi_display.monitor);
...
//		printf("3. overscan_x = %d, overscan_y = %d, sunxi_display.monitor = %d\n", overscan_x, overscan_y, sunxi_display.monitor);
...
//printf("4. overscan_x = %d, overscan_y = %d, sunxi_display.monitor = %d, mode->xres = %d, mode->yres = %d \n", overscan_x, overscan_y, sunxi_display.monitor, mode->xres, mode->yres);
	...
	sunxi_display.fb_size = (mode->xres * mode->yres * 5 + 0xfff) & ~0xfff;
	...
	memsetl(gd->fb_base, 800 * 600, 50);			//未使能LCD之前先清屏一次
	printf("1. draw ---------->");
	mdelay(5000);
	memsetl(gd->fb_base, 800 * 600, 50);			//未使能LCD之前先清屏一次
	printf("2. draw ---------->");
	...
	sunxi_display.fb_size =
		(mode->xres * mode->yres * 5 + 0xfff) & ~0xfff;
	overscan_offset = (overscan_y * mode->xres + overscan_x) * 4;
...
/*
printf("mode->xres = %d, mode->yres = %d, mode->hsync_len= %d, mode->left_margin= %d, mode->lower_margin= %d\n", mode->xres, mode->yres, mode->hsync_len, mode->left_margin, mode->lower_margin);
printf("mode->pixclock = %d, mode->pixclock_khz= %d, mode->refresh= %d, mode->right_margin= %d, mode->sync= %d\n", mode->pixclock, mode->pixclock_khz, mode->refresh, mode->right_margin, mode->sync);
printf("mode->upper_margin = %d, mode->vmode= %d, mode->vsync_len= %d\n", mode->upper_margin, mode->vmode, mode->vsync_len);
*/
...
//	printf("gd->bd->bi_dram[0].start =  0x%x, gd->bd->bi_dram[0].size =  0x%x, gd->fb_base = 0x%x, fb_dma_addr =0x%x, sunxi_display.fb_addr = 0x%x\n", gd->bd->bi_dram[0].start, gd->bd->bi_dram[0].size, gd->fb_base, fb_dma_addr, sunxi_display.fb_addr);
